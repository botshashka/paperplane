...
class TestCommandTest {
    ...
    fun `parses passing test from JUnit XML`() {
        writeXml("TEST-com.example.FooTest.xml", """
    ...
    fun `parses mix of passing and failing tests`() {
        writeXml("TEST-com.example.MixedTest.xml", """
    ...
    fun `stale XML files are cleaned before run`() {
        writeXml("TEST-com.example.OldTest.xml", """
        ...
        writeXml("TEST-com.example.NewTest.xml", """
    ...
    fun `verbose failure preserves full stack trace`() {
        val rawFailure = """org.example.SomeException: broke
            at com.example.Test.method(Test.java:10)
            at com.example.Test.run(Test.java:5)""".trimIndent()
