 ...
 * [PaperServerManager.start] call — initial startup, fix recovery, leak restart, awaiting-fix
 * cold start, blue-green standby, pre-warm and fixed-server alike. This makes the "mirror the
 * args" invariant structural: no recovery or restart path can silently downgrade a server to a
 * plain JVM without the redefinition agent or the JBR flags, because there is exactly one place
 * the launch arguments are assembled.
 */
data class LaunchSpec(
  ...
  companion object {
     ...
     * `URLClassLoader.addURL` reflectively on Paper's jar-backed plugin classloader, which
     * JDK 17+ rejects with `InaccessibleObjectException` unless this open is present at launch.
     */
