...
object TerminalUI {
    ...
    /**
     * Discards the current pinned block without printing it, regardless of type.
     * Use in shutdown hooks where no new block follows.
     */
    fun discardBlock() {
        lock.withLock {
            if (!blockActive && blockLines.isEmpty()) return
    ...
    /**
     * Ends the current block and starts a transient "Watching/Waiting" block.
     * Convenience for the pattern that repeats throughout DevCommand.
     */
     ...
     * When no block is active, prints directly.
     */
    ...
    /**
     * Updates the detail text shown after the spinner message on the same line.
     * Only meaningful while a [spin] block is executing; otherwise falls back to [status].
     */
    ...
    /**
     * Runs [block] while showing a spinner with [message] pinned in the footer.
     * Server/proxy logs scroll above; the spinner stays at the bottom.
     * Code inside [block] can call [spinSubstatus] to update detail text.
     */
