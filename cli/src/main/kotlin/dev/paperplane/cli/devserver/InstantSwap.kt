 ...
 * atomically — or refuse. Class bytes travel on the wire (base64) rather than as filesystem paths:
 * payloads are small (changed classes only) and the message stays valid when the server has no
 * shared filesystem (the future containerized Fresh engine).
 *
 ...
 * loaded; the companion refuses on mismatch instead of patching over drift.
 */
 ...
 * The baseline advances for exactly these — never for everything requested — so a class the
 * companion skipped can never be recorded as patched.
 *
 ...
 * (baseline drift, plugin not loaded, leak limit); nothing was touched. `failed` — the JVM vetoed,
 * the attempt errored, or a refusal arrived after a side effect had already landed; redefineClasses
 * is all-or-nothing, so no patch was applied either way. Refused and failed both send the CLI down
 * the mode's full swap path.
 */
