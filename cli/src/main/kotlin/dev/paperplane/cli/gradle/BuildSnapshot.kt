...
/**
 * Tracks .class file hashes between builds to detect what changed.
 * Used to determine whether a change is suitable for hot-swap (Level 2)
 * vs requiring a full classloader reload (Level 1).
 */
class BuildSnapshot(private val classesDir: File) {
    ...
    companion object {
        fun diff(previous: Map<String, Long>, current: Map<String, Long>): ClassChanges {
            val modified = current.keys.intersect(previous.keys)
