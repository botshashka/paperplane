...
class DevCommand : CliktCommand(name = "dev") {
  ...
  private fun runBlueGreen(
    ...
    val proxyReady =
        TerminalUI.spin("Starting Velocity proxy...") { velocityManager.waitForReady(25565) }
  ...
  /**
   * Pre-warms a standby server by syncing state and starting it in the background. This way it's
   * already booted when the next rebuild needs it, saving cold-start time.
   */
  ...
  private fun hotReloadRebuild(
    ...
    if (fastMeta != null && fastMeta.classesDir.isNotEmpty()) {
      ...
      TerminalUI.info(
          "Strategy:",
          if (strategy == "hotswap") "hotswap (${changes.modified.size} modified)"
