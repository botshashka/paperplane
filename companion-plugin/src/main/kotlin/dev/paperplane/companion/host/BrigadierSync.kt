 ...
 *
 * Calls `CraftServer#syncCommands()` reflectively (it's not on the Bukkit `Server` interface). That
 * call rebuilds the server-side Brigadier dispatcher AND pushes `ClientboundCommandsPacket` to
 * every online player, so we don't need a separate per-player `updateCommands()` loop — adding one
 * races with Paper's own send on the shared dispatcher tree and produces a `CME`.
 *
