...
class PluginReloader(
    ...
    fun reload(pluginName: String, jarFile: File, rollbackJar: File? = null): ReloadOutcome {
        val rollbackFn = if (rollbackJar != null && rollbackJar.exists() && rollbackJar.absolutePath != jarFile.absolutePath) {
    ...
    /**
     * Reloads a plugin from build output directories instead of a JAR.
     * Skips JAR packaging entirely — loads classes from build/classes/ and
     * dependencies from resolved runtime classpath JARs.
     */
    fun reloadFromDirectory(
        ...
        val rollbackFn = if (rollbackJar != null && rollbackJar.exists()) {
            { loadAndEnableFromJar(rollbackJar, pluginName) }
        } else null
        return reloadWithStrategy(pluginName, { loadAndEnableFromDirs(buildOutputDirs, pluginName) }, rollbackFn)
    }
    ...
    private fun loadAndEnableFromJar(jarFile: File, pluginName: String): Plugin {
        ...
        val (pluginInstance, classLoaderInstance) = PaperInternals.loadPluginFromJar(jarFile, description, server, logger)
    ...
    private fun loadAndEnableFromDirs(dirs: List<String>, pluginName: String): Plugin {
        ...
        val resourcesDir = dirs.firstOrNull { File(it, "plugin.yml").exists() }
        ...
        PaperInternals.initializePlugin(pluginInstance, description, dataFolder, classLoader, server, logger)
