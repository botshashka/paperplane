 ...
 *
 * Design principle: trust the JVM, not our own bytecode analysis.
 * We attempt redefineClasses() and catch UnsupportedOperationException
 * for structural changes. The optional ClassChangeDetector pre-filter
 * avoids the exception cost but is not required for correctness.
 */
class HotSwapper(private val logger: Logger) {
    ...
    /**
     * Whether JetBrains Runtime enhanced redefinition is available.
     * When true, structural changes (new methods/fields) can be hot-swapped.
     */
    ...
    private fun resolveInstrumentation(): Instrumentation? {
        return try {
            val agentClass = ClassLoader.getSystemClassLoader().loadClass("dev.paperplane.agent.PaperPlaneAgent")
