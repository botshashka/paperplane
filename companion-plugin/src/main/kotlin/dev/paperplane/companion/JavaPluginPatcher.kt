...
/**
 * Replaces the entire body of JavaPlugin.<init>()V with a safe version that skips the
 * PluginClassLoader check instead of throwing.
 */
private class RewrittenInitVisitor(mv: MethodVisitor, private val superName: String) :
  ...
  override fun visitInsn(opcode: Int) {
    if (!started) super.visitInsn(opcode)
  }

  override fun visitIntInsn(opcode: Int, operand: Int) {
    if (!started) super.visitIntInsn(opcode, operand)
  }

  override fun visitVarInsn(opcode: Int, `var`: Int) {
    if (!started) super.visitVarInsn(opcode, `var`)
  }

  override fun visitTypeInsn(opcode: Int, type: String?) {
    if (!started) super.visitTypeInsn(opcode, type)
  }

  override fun visitFieldInsn(opcode: Int, owner: String?, name: String?, descriptor: String?) {
    if (!started) super.visitFieldInsn(opcode, owner, name, descriptor)
  }
  ...
  override fun visitMethodInsn(
    ...
    if (!started) super.visitMethodInsn(opcode, owner, name, descriptor, isInterface)
  }

  override fun visitJumpInsn(opcode: Int, label: Label?) {
    if (!started) super.visitJumpInsn(opcode, label)
  }

  override fun visitLabel(label: Label?) {
    if (!started) super.visitLabel(label)
  }

  override fun visitLdcInsn(value: Any?) {
    if (!started) super.visitLdcInsn(value)
  }

  override fun visitIincInsn(`var`: Int, increment: Int) {
    if (!started) super.visitIincInsn(`var`, increment)
  }

  override fun visitTableSwitchInsn(min: Int, max: Int, dflt: Label?, vararg labels: Label?) {
    if (!started) super.visitTableSwitchInsn(min, max, dflt, *labels)
  }

  override fun visitLookupSwitchInsn(dflt: Label?, keys: IntArray?, labels: Array<out Label>?) {
    if (!started) super.visitLookupSwitchInsn(dflt, keys, labels)
  }

  override fun visitMultiANewArrayInsn(descriptor: String?, numDimensions: Int) {
    if (!started) super.visitMultiANewArrayInsn(descriptor, numDimensions)
  }

  override fun visitTryCatchBlock(start: Label?, end: Label?, handler: Label?, type: String?) {
    if (!started) super.visitTryCatchBlock(start, end, handler, type)
  }
  ...
  override fun visitFrame(
    ...
    if (!started) super.visitFrame(type, numLocal, local, numStack, stack)
  }

  override fun visitMaxs(maxStack: Int, maxLocals: Int) {
    /* already emitted in visitCode */
  }
