 ...
 *
 * Every reflective access point is in this file so that when Paper changes
 * internal class layouts (constructor signatures, field names, wrapper structures),
 * only this file needs updating. Resolution results are cached per session.
 */
...
object PaperInternals {
    ...
    /**
     * Creates a plugin instance from a JAR file using Paper's PluginClassLoader via reflection.
     * This is the JAR-based loading path — directory loading bypasses this entirely.
     */
    fun loadPluginFromJar(
        ...
        initializePlugin(pluginInstance, description, dataFolder, classLoaderInstance as ClassLoader, server, logger, jarFile)

        return pluginInstance to classLoaderInstance
    }

    private fun resolvePluginClassLoaderConstructor(pclClass: Class<*>): Constructor<*> {
        pluginClassLoaderCtor?.let { return it }

        val ctor = pclClass.declaredConstructors
    ...
    /**
     * Initializes a JavaPlugin instance. Tries JavaPlugin.init() first,
     * falls back to direct field injection.
     */
    ...
    private fun tryInit(
        ...
        return try {
            ...
            val loader = JavaPluginLoader(server)
            initMethod.invoke(plugin, loader, server, description, dataFolder, jarFile ?: dataFolder, classLoader)
            true
        } catch (_: Exception) {
            false
        }
    ...
    fun registerInPluginManager(server: Server, plugin: Plugin, pluginName: String) {
        val spm = unwrapSimplePluginManager(server)
            ?: throw IllegalStateException("Could not find SimplePluginManager (Paper $paperVersion)")
     ...
     */
    fun registerCommands(server: Server, plugin: JavaPlugin, description: PluginDescriptionFile, logger: Logger) {
        try {
            val commandMap = server.commandMap
            for ((name, cmdInfo) in description.commands) {
                val command = PluginCommand::class.java
    ...
    /**
     * Updates Paper's internal plugin registry (separate from SimplePluginManager)
     * to point to a new plugin instance after reload.
     */
    fun updatePaperPluginRegistry(server: Server, pluginName: String, newPlugin: Plugin, logger: Logger) {
        var updated = false
        ...
        if (!updated) {
            logger.warning("Could not find plugin '$pluginName' in Paper's internal registry (Paper $paperVersion)")
        }
    }
}
