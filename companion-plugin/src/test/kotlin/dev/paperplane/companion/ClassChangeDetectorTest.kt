...
class ClassChangeDetectorTest {
  ...
  fun `identical bytecode is method-body-only`() {
    val bytes = generateClass(methods = listOf(Triple(Opcodes.ACC_PUBLIC, "foo", "()V")))
  ...
  fun `same structure with different method body is method-body-only`() {
    val oldBytes =
        generateClass(methods = listOf(Triple(Opcodes.ACC_PUBLIC, "foo", "()V"))) {
            cw,
            access,
            mname,
            desc ->
  ...
  fun `adding a method is structural`() {
    ...
    val new = generateClass(methods = listOf(Triple(Opcodes.ACC_PUBLIC, "newMethod", "()V")))
    assertFalse(detector.isMethodBodyOnly(old, new))
  }

  @Test
  fun `removing a method is structural`() {
    val old = generateClass(methods = listOf(Triple(Opcodes.ACC_PUBLIC, "foo", "()V")))
  ...
  fun `changing method descriptor is structural`() {
    val old = generateClass(methods = listOf(Triple(Opcodes.ACC_PUBLIC, "foo", "()V")))
    val new = generateClass(methods = listOf(Triple(Opcodes.ACC_PUBLIC, "foo", "(I)V")))
    assertFalse(detector.isMethodBodyOnly(old, new))
  }
  ...
  fun `adding a field is structural`() {
    val old = generateClass()
    val new = generateClass(fields = listOf(Triple(Opcodes.ACC_PRIVATE, "x", "I")))
    assertFalse(detector.isMethodBodyOnly(old, new))
  }

  @Test
  fun `removing a field is structural`() {
    val old = generateClass(fields = listOf(Triple(Opcodes.ACC_PRIVATE, "x", "I")))
    val new = generateClass()
    assertFalse(detector.isMethodBodyOnly(old, new))
  }
  ...
  fun `changing field type is structural`() {
    val old = generateClass(fields = listOf(Triple(Opcodes.ACC_PRIVATE, "x", "I")))
    val new = generateClass(fields = listOf(Triple(Opcodes.ACC_PRIVATE, "x", "J")))
    assertFalse(detector.isMethodBodyOnly(old, new))
  }
  ...
  fun `changing method access modifier is structural`() {
    val old = generateClass(methods = listOf(Triple(Opcodes.ACC_PUBLIC, "foo", "()V")))
    val new = generateClass(methods = listOf(Triple(Opcodes.ACC_PRIVATE, "foo", "()V")))
    assertFalse(detector.isMethodBodyOnly(old, new))
  }
}
